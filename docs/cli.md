...
| `--moe-bank-prefetch` | off | Experimental. With `--moe-bank-ram` and cpu/hybrid decode, Linux: before the CPU executor computes a layer, it asks the kernel for exactly the non-resident expert rows that layer routes to (`mincore`, then `MADV_WILLNEED` per row), instead of leaving them to 4 KiB faults and readahead windows. Not yet measured end to end. See [bank-ram.md](bank-ram.md#6-experimental-ask-for-the-rows-each-step-routes-to---moe-bank-prefetch) |
