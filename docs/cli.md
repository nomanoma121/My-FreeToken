...
| `--mm-encoder-weights` | host | Where the encoder tower's block weights live. `host` streams them from pinned host banks two blocks at a time behind the compute (about 60 MiB of VRAM instead of the whole tower; small images encode slower, about 17 ms instead of 7 ms for 448x448); `gpu` keeps them resident. `cpu` (Kai): the vision tower runs on the CPU in the tokenizer worker, with no VRAM and no pinned memory, a few seconds per image; Qwen3.5/3.6 and Qwen3.8-Flash-Next, see [image-input.md](image-input.md) |
| `--mm-encoder-dtype` | auto | (Kai) Compute dtype of a GPU encoder tower. `auto` builds it in float32 when the model runs bfloat16 and in the model dtype otherwise: the Qwen VL vision tower loses about 9% of its output in bfloat16 (see [image-input.md](image-input.md)). `float32`, `float16`, `bfloat16` force one |
