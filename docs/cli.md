...
| `--linear-state-cache-ratio` | 2.0 | Hybrid GDN models: GDN-state snapshots kept for prefix reuse, per running request (floor 4). This, not KV, bounds how many conversations stay reusable — about max(4, ratio) ÷ 2 with `--max-running-req 1`; each slot is one GDN state of VRAM taken from the expert cache. See [prefix-reuse.md](prefix-reuse.md) |
...
| `--prefill-chunk-budget` | 0.55 | Share of free VRAM one prefill chunk's transient may take. The engine measures that cost per token at startup, sizes `--max-prefill-length` to fit, and re-solves before every prefill against the VRAM free right then. 0 disables and the flag is used as given. Under `--pp-size` the chunk is agreed across the ranks at startup and then frozen, because it sizes a cross-rank message. See [prefill-chunk.md](prefill-chunk.md) |
