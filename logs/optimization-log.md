...
### LRU eviction カーネルの改造を再検討 → 却下 (スコープ外の根本理由を明確化)

キャッシュサイズを増やせないなら、同じサイズでミス率を下げる
(LRUよりスマートなeviction方策、例えば頻度加味のLFU的な方策) 余地は
ないか、`flashlib`の`lru_ensure`カーネル (`_lru_ensure_kernel`/
`_lru_ensure_insert_kernel`, 441行) を再度読み直して検討した。

以前のセッションでは「レースコンディション履歴やCUDA-graph安全性を
理由にリスクが高い」として見送っていたが、再検討の結果、それ以上に
**構造的な理由**でスコープ外と判断した: このファイルは
`~/My-FreeToken/.venv/lib/python3.12/site-packages/flashlib/` 配下の
**pipインストール済みの外部パッケージ**であり、FreeToken本体のgit管理下
にあるソースツリーの外にある。ここを書き換えても:

1. `~/My-FreeToken`のgit historyには一切残らない (ユーザーが明示的に
   要求している「必ず作業はgitで管理すること」という制約に反する)
2. `uv pip install -e ".[accel]"`の再実行や環境再構築で書き換えが
   跡形もなく消える (再現性がない)

コード自体の複雑さ・リスク以前に、「この変更はFreeToken本体への
改造として成立しない」という理由で不採用と判断する。もし本気で
eviction方策を変えるなら、flashlibをフォークしてFreeTokenのリポジトリに
ベンダリングする必要があるが、これは「FreeTokenを最適化する」という
依頼のスコープを超えて「サードパーティ依存を丸ごと持ち込んで改造する」
規模の話になり、着手しない。

## `--ple-backend pinned` を試したが起動時にハング (負の結果、不採用)

これまで未検証だった`--ple-backend`(デフォルト`disk`: PLE n-gram表を
チェックポイントファイルからio_uring/O_DIRECTで都度読む) を`pinned`
(47.7GBの表全体をpage-lockedホストRAMに事前ロード) に変更すれば、
decode時の毎ステップのディスクI/Oが消え、追加の高速化が見込めるのでは
ないかと考え検証した。システムRAMは121GiB (117GiB空き) あり、47.7GBの
テーブルは理論上問題なく載る想定だった。

### 結果: 初回はgloo 60秒タイムアウトで失敗、300秒に延長しても失敗 (ハング)

1回目 (`--distributed-timeout`未指定、デフォルト60秒): PLE表のpinロード
自体は約38秒で完了したが、rank0側のMoEエキスパートバンク読み込みも
重なり、rank1が60秒タイムアウトでKVページ数合意に失敗して起動失敗。
これは既知の「`--distributed-timeout`で解決可能な純粋なタイミング問題」
パターンと判断し、`--distributed-timeout 300`を追加して再試行。

2回目 (`--distributed-timeout 300`): PLE表のpin自体は約36秒で完了したが、
その後 **rank1側のエキスパートバンク読み込みが6シャード中3シャード
(50%)で完全に停止し、残り約5分間 (300秒のタイムアウトいっぱい) 一切
進捗しないままタイムアウトした**。単なる「遅い」ではなく実質的な
ハング。`free -h`で確認したところ、失敗後にswap使用量が1.1GiB→1.5GiBへ
増加しており、47.7GBのpin常駐化とMoEエキスパートバンクの一時的な
ホストRAM使用 (これまでも「low free RAM -> serial build」警告が頻発
していた) が重なり、ページキャッシュの奪い合い・スワッシングを
引き起こしたと推測される。

### 結論: 不採用

`--ple-backend pinned`はこのシステムのRAM予算・I/O帯域とは相性が悪く、
起動を不安定化させる (最悪ハングする) ことが実測で確認された。理論上の
「decode毎ステップのディスクI/Oを消せる」という利点よりも、実運用上の
...
## 最後の検討: FreeToken自前のTritonカーネル (QSA attention) はチューニング済みか

`--attention-backend`に代替がないなら、`qsa_sparse`カーネル自体
(`python/freetoken/kernel/triton/qsa/attend.py`、flashlibと違いFreeToken
本体のgit管理下にある) にチューニングの余地がないか、コードを直接確認した。

このカーネルには`_profile_ladder`という適応的フォールバック機構があり、
「チューニング済みprofile (BLOCK_N, num_stages, num_warps)」から開始し、
デバイスの共有メモリに収まらない場合は段階的にタイルを縮小する
(`_LADDER_DESC: BLOCK_N/num_stages: (n,2) -> (n,1) -> (n/2,2) -> ...`)。
フォールバックが発動すると`qsa_sparse: BLOCK_N=... (tuned profile was
BLOCK_N=...)`という警告ログが出る設計になっている。

**このセッション中に起動した全サーバーログ (`logs/serve_*.log`、数十回の
起動) を`grep`したところ、この警告は一度も出ていない** ──
つまり、我々の全ての実験は最初から最後まで、このカーネルの
「チューニング済みprofile」をそのまま使えており、共有メモリ不足による
劣化フォールバックには一度も陥っていなかったことが確認できた。

これは重要な意味を持つ: この自前カーネルは既に狙い通りの設定で動作して
おり、「実は変な設定にフォールバックしていて、それを直せば速くなる」
という都合の良い抜け道は存在しない。ここから更に速くするには、
既にベンチマークされ選定済みのBLOCK_N/warps/stagesの組み合わせを
上回る新しい設定を見つける必要があり、これは適切なプロファイラ
(Nsight Compute等) を使った反復的な実測が必要な、正真正銘の
低レベルカーネルエンジニアリングである。これまでのセッションで
行ってきた「ルーティング・設定・データロード」レベルの改造とは
質的に異なる規模の作業であり、既に何度も述べた「カーネルレベルの
書き換えが必要」という結論を、憶測ではなく実際のログ精査によって
裏付けるものとなった。

## 実際にカーネルレベルの調整を1件試した: QSA attentionのsplit-k次数

「カーネルレベルの書き換えが必要」という結論に留まらず、実際に1件、
FreeToken本体 (git管理下、flashlibのような第三者パッケージではない)
のTritonカーネルパラメータを調整する実験を行った。

`qsa_sparse_paged_attention` (`python/freetoken/kernel/triton/qsa/attend.py`)
のsplit-k並列度を決める箇所に次のコメントを発見:

> "Tuned on GB300 for the Qwen-Air TP1, TP2, and TP4 attention shapes."

GB300 (データセンター向けBlackwell Ultra) と RTX 3060 (28 SM, Ampere) は
SM数が大きく異なる。我々のdecode呼び出し (batch=1, kv_heads=2 →
`base_programs=2`) は`target_splits=64`の分岐に該当し、
`indexer_budget=2048 / indexer_compress_ratio=4=512` (TOPK) と`BLOCK_N=16`
から`num_tiles=32`、実際の`num_splits=min(32,64)=32`。つまりQSA層1回の
呼び出しで`batch(1) x kv_heads(2) x splits(32) = 64`個の小さなカーネル
プログラムが起動される計算になる。SM数の少ない3060では、この細粒度
並列化がカーネル起動オーバーヘッド超過を招いている可能性を仮説とし、
実測で検証した。split-kの分割数は正しさに影響しない(mergeカーネルが
分割数に関わらず正しく統合する設計)ため、品質リスクなく調整・実測できる。

### 実測 (sched-h構成、`target_splits`のみ変更、algebra/riddle/codeの品質は
全条件で正常)

| target_splits | decode tok/s (prose/code) |
|---|---|
| 64 (元のGB300向けチューニング値) | 36-38.4 (基準範囲) |
| 8 | 35.88-36.71 / 36.55-37.37 (基準範囲内、有意差なし) |
| 1 (split-kなし、完全直列) | **30.14 / 30.87 (明確に悪化)** |

### 結論: 仮説は反証された。GB300向けの値は3060でも妥当

target_splits=8は64と有意差がない一方、1まで下げると明確に悪化した。
「並列度を下げるほどオーバーヘッドが減って速くなる」という単純な仮説は
誤りで、ある程度の並列度 (SM占有率確保) は3060でも引き続き有効だと
分かった。64という値はこのGPUでも既に妥当な範囲にある。

これは「試していないので分からない」の解消ではなく、実際にコードを
変更・計測した上での反証であり、QSAのsplit-k次数はこのハードウェアの
ボトルネックではないことを実測で確定した。変更は元の値に戻して破棄
(`git diff`で無変更を確認済み、コミットなし、実験のみ)。この結果は、
真のカーネルレベル最適化 (BLOCK_N/warps/stagesの根本的な再チューニング)
にはNsight Compute等の適切なプロファイラによる反復測定が必要で、
...
## dense GEMVカーネルのタイル形状 (BLOCK_N/BLOCK_K) 自体も実測で調整を試みた

Nsight Computeがなくても、末端のtok/sをものさしにタイル形状を直接
変更・計測することはできる、と考え直し、`_gemv_splitk_kernel`の
`BLOCK_N`/`BLOCK_K`自体 (単なる並列度パラメータではなく、1プログラムが
担当する仕事の単位そのもの) を変えて実測した。

| 変更 | decode tok/s (prose/code) | 品質 |
|---|---|---|
| (基準) BLOCK_N=16, BLOCK_K=128, num_warps=1 | 36-38.4 | 正常 |
| BLOCK_N=32 (単独、num_warps=1のまま) | 36.47 / 37.50 | 正常、基準範囲内 |
| **BLOCK_N=64, BLOCK_K=256 (同時に倍増、num_warps=1のまま)** | **10.78 / 10.96 (約3.5倍の劣化)** | 正常 (速度のみ劣化) |
| BLOCK_N=32 + num_warps=2 (両方バランスよく倍増) | 36.44 / 37.52 | 正常、基準範囲内 |

BLOCK_N=64+BLOCK_K=256の組み合わせで**明確な3.5倍の速度劣化**を確認した
(品質は正常なので、単なる並列度不足によるもの)。num_warps=1のまま
1プログラムが担当するタイル面積を8倍 (4×2) に増やしたことで、32
スレッド(1ワープ)あたりの逐次処理量が大幅に増え、深刻な並列度不足に
陥ったと考えられる。一方、BLOCK_Nだけの倍増や、BLOCK_N・num_warps両方を
バランスよく倍増させた場合は速度に有意な変化がなかった。

### 結論: タイル形状には確かに感度があるが、単純な試行錯誤では「壊さない」以上のことは分からない

この実験は「dense GEMVカーネルのタイル形状パラメータには全く感度がない」
という誤った結論を防いだという意味で価値があった (num_warps/split-kだけを
見ていたら「感度なし」と誤認しかねなかった)。しかし同時に、
BLOCK_N/BLOCK_K/num_warps/num_stagesの多次元空間を手動の試行錯誤
(2分ごとの再起動サイクル) で探索するのは非効率かつリスクが高い
(今回のように3.5倍の劣化を引き当てる可能性がある) ことも実証された。
6パターン試して分かったのは「小さな変更は無害、大きな変更は
バランスを崩すと大幅に悪化する」という定性的な傾向のみで、
「元の16/128/1/1536という値を上回る具体的な改善」には到達できなかった。

これは、真にこのカーネルを改善するには、Nsight Compute等で
occupancy・メモリ帯域利用率などの指標を見ながら系統的に探索する必要が
あるという結論を、より強く裏付ける結果となった。全ての変更は元の値に
戻して破棄 (`git diff`で無変更を確認済み、コミットなし)。
...
### Takeover (Muse Spark, 2026-09-16): --moe-cpu-threads 24 も効果なし
...
0/12/24 の3点が全て誤差範囲に収束。CPUスレッド数レバーは打ち止め。

### Takeover (Muse Spark, 2026-09-16): --pp-layers 28 も効果なし (top-10)

| --pp-layers | prose | code |
|---|---|---|
| 30 | 24.15 | 24.75 |
| 28 | 24.55 | 24.80 |
...
### Takeover (Muse Spark, 2026-09-16): --pp-layers 32 も効果なし (top-10)
...
28/30/32 の3点が全て24-25に収束。層配分レバーはtop-10でも頭打ち。

### Takeover (Muse Spark, 2026-09-16): ncu初計測 + 27B最適化 + nsys通信解析 (完全記録)


> Qwen27B (llama.cpp) 側の記録は `logs/optimization-log-qwen27b.md` に分離した。
> 以下は3.8 (FreeToken) 側のncu計測のみ残す。

#### 1. ncuによるdense FP8 GEMV直接計測 (FreeToken, 3.8, top-10系)

自作マイクロベンチ (`/tmp/ncu_gemv.py`, 後に削除可) で `block_fp8_linear` の
decodeパス (M==1, W8A16 split-K GEMV) を `ncu` で計測。ncuはreboot後の
`NVreg_RestrictProfilingToAdminUsers=0` 反映により一般ユーザー権限で動作確認済み。
注意: `block_fp8_matmul` (GEMM) を直接呼ぶと当該triton版では
`fp8e4nv not supported` でコンパイル失敗する。decodeは `block_fp8_linear` 経由で
`_block_fp8_gemv_splitk_kernel` を使うのが正規パス。

代表形状 N=7680, K=2560 の結果:
- `dram__throughput`: ピーク比 **82.2%** (メモリバウンド確定)
- `sm__throughput`: 49.0%, `sm__warps_active`: **30.7%** (レジスタ圧迫の疑い)
- active 111k cycles / elapsed 124k cycles (89%)
